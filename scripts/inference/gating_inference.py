...
def _load_gating_model(ckpt_path: Path, device: torch.device) -> tuple[PatchConvGate2D, float]:
    ...
    cfg = PatchGatingConfig(
        num_experts=int(gc["num_experts"]),
        num_classes=int(gc["num_classes"]),
        image_channels=int(gc.get("image_channels", 3)),
        patch_size=int(gc["patch_size"]),
        stride=int(gc["stride"]),
        hidden_dim=hidden_dim,
        score_hidden_dim=int(gc.get("score_hidden_dim", hidden_dim)),
        context_hidden_dim=int(gc.get("context_hidden_dim", 32)),
        dropout=float(gc.get("dropout", 0.1)),
        per_class=bool(gc.get("per_class", False)),
        use_residual_head=bool(gc.get("use_residual_head", True)),
        use_entropy=bool(gc.get("use_entropy", True)),
        use_consensus_features=bool(gc.get("use_consensus_features", True)),
        use_disagreement_features=bool(gc.get("use_disagreement_features", True)),
        use_confidence_features=bool(gc.get("use_confidence_features", True)),
        use_prior_agreement_features=bool(gc.get("use_prior_agreement_features", False)),
        use_layer1_semantics=bool(gc.get("use_layer1_semantics", False)),
        use_image_context=bool(gc.get("use_image_context", False)),
        use_position_channels=bool(gc.get("use_position_channels", False)),
        use_slice_position=bool(gc.get("use_slice_position", False)),
        use_context_film=bool(gc.get("use_context_film", True)),
        blend_mode=str(gc.get("blend_mode", "gaussian")),
...
def _extract_extra_patch(
    extra_maps: dict[str, np.ndarray | float] | None,
    y: int,
    x: int,
    patch_size: int,
    ...
    for key, value in extra_maps.items():
        if key == "slice_pos":
            patch[key] = torch.tensor([float(value)], dtype=torch.float32)
            continue
        arr = np.asarray(value, dtype=np.float32)
        patch[key] = torch.from_numpy(arr[..., y : y + patch_size, x : x + patch_size]).float()
    return patch
...
def infer_single_image(
    ...
    *,
    ...
    for y, x in positions:
        ...
        for key, value in _extract_extra_patch(extra_maps, y, x, patch_size).items():
            extra_lists.setdefault(key, []).append(value)
    ...
    with torch.no_grad():
        weights = model(
            logits_batch,
            extra=extra_batch if extra_batch else None,
    ...
    if weights_np.ndim == 3:
        ...
        flat_patches = [
            np.broadcast_to(weights_np[i][:, :, None, None], (k, m, patch_size, patch_size))
            .copy()
            .reshape(k * m, patch_size, patch_size)
            for i in range(len(positions))
        ]
        merged = merge_patches_2d(
            flat_patches,
            positions,
            (height, width),
            patch_size,
        ...
        weight_map_per_class = merged.reshape(k, m, height, width)
    else:
...
def main() -> None:
    ...
    expected_experts = len(list_experts(models_cfg))
    fold = int(args.fold)
    ...
    model, ckpt_temperature = _load_gating_model(ckpt_path, device)

    if model.cfg.num_classes != num_classes:
        raise ValueError(
            f"Gating checkpoint expects num_classes={model.cfg.num_classes}, but dataset has {num_classes}"
        )
    if model.cfg.num_experts != expected_experts:
        raise ValueError(
            f"Gating checkpoint expects num_experts={model.cfg.num_experts}, but models config has {expected_experts}"
        )
    ...
    for sample in tqdm(eval_rows, desc=f"gating inference {split}"):
        ...
        prob_path = get_oof_prob_path(l2_oof_map, sid)
        with np.load(prob_path) as probs_data:
            if "logits" in probs_data:
                logits_arr = probs_data["logits"].astype(np.float32)
            elif "probs" in probs_data:
                probs = probs_data["probs"].astype(np.float32).clip(1e-6, 1 - 1e-6)
                logits_arr = np.log(probs / (1 - probs))
            else:
                raise KeyError(f"OOF cache missing 'logits'/'probs': {prob_path}")
        ...
        fused, weight_map, weight_map_per_class = infer_single_image(
            ...
            device,
            temperature=temperature,
            extra_maps=extra_maps if extra_maps else None,
        )
        ...
        if args.save_weights:
            ...
            if args.save_weight_png:
                ...
                for expert_idx in range(weight_map.shape[0]):
                    ...
                    Image.fromarray(w_uint8).save(png_dir / f"{sid}_expert{expert_idx}.png")
    ...
    if all_metrics:
        ...
        print(f"No evaluable samples found for split={split}")
