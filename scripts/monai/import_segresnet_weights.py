...
def _resolve_segresnet_expert(
    experts: List[Dict[str, Any]],
    requested_name: str | None,
) -> tuple[Dict[str, Any], str]:
    seg_types = {"monai_segresnet", "monai_segresnet_ds"}
    candidates = [e for e in experts if e.get("type", "").lower() in seg_types]
    ...
    if requested_name:
        ...
        raise ValueError(
            f"Requested expert '{requested_name}' not found among SegResNet experts: "
    ...
    raise ValueError(
        "Multiple SegResNet experts found. Please specify --expert-name explicitly. "
...
def main() -> None:
    ...
    seg_cfg, seg_name = _resolve_segresnet_expert(experts, args.expert_name)
